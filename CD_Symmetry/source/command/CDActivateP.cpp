...
class CDActivatePositive : public CommandData
{
	public:
...
Bool CDActivatePositive::Execute(BaseDocument* doc)
{
...
class CDActivatePositiveR : public CommandData
{
	public:
...
Bool RegisterCDActivatePositive(void)
{
	...
	String name=GeLoadString(IDS_CDPACTIVE); if (!name.Content()) return true;

	if(!reg) return CDRegisterCommandPlugin(ID_CDACTIVATEPOSITIVE,name,PLUGINFLAG_HIDE,"CDActivateP.tif","CD Activate Positive",CDDataAllocator(CDActivatePositiveR));
	else return CDRegisterCommandPlugin(ID_CDACTIVATEPOSITIVE,name,0,"CDActivateP.tif","CD Activate Positive",CDDataAllocator(CDActivatePositive));
}
