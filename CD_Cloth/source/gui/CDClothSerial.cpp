...
Bool RegisterCDCL(void)
{
	...
	if(!CDReadPluginInfo(ID_CDCLOTH,data,CDCL_SERIAL_SIZE)) return false;
	
	cdsknr.SetCString(data,CDCL_SERIAL_SIZE-1);
	if(!CheckKeyChecksum(cdsknr)) return false;
