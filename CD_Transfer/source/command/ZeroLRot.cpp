	...
	for(i=0; i<opCount; i++)
	{
		...
		if(op)
		{
			...
			else
			{
				...
				{
					BaseContainer *tData = op->GetTag(ID_CDZEROTRANSTAG)->GetDataInstance();
					if(tData)
					{
						if(shift)
						{
...
{
	if(CDFindPlugin(ID_ABOUTCDIKTOOLS,CD_COMMAND_PLUGIN) && CDFindPlugin(ID_ZEROLOCALROTATION,CD_COMMAND_PLUGIN)) return true;
	else
	{
		Bool reg = true;
