...
{
	if(CDFindPlugin(ID_ABOUTCDJOINTSKIN,CD_COMMAND_PLUGIN) && CDFindPlugin(ID_LINKOBJECTS,CD_COMMAND_PLUGIN)) return true;
	else
	{
		Bool reg = true;
		...
		if(!reg) return CDRegisterCommandPlugin(ID_LINKOBJECTS,name,PLUGINFLAG_HIDE,"CDLinkObjects.TIF","CD Link Objects",CDDataAllocator(CDLinkObjectsR));
		else return CDRegisterCommandPlugin(ID_LINKOBJECTS,name,0,"CDLinkObjects.TIF","CD Link Objects",CDDataAllocator(CDLinkObjects));
	}
}
