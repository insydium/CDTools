	...
	if(res)
	{
		...
		GroupBegin(0,BFH_SCALEFIT,1,0,"",0);
		{
			...
			GroupBegin(0,BFH_SCALEFIT,1,0,GeLoadString(DLG_CDJS_OPTIONS),0);
			{
				...
				AddChild(IDC_MIRROR_AXIS, 0, GeLoadString(IDS_JOINTS));
				AddChild(IDC_MIRROR_AXIS, 1, GeLoadString(IDS_BONES));
...
{
	if(CDFindPlugin(ID_CDCONVERTFROMCDJOINTS,CD_COMMAND_PLUGIN)) return true;
