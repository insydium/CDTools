...
{
	if(CDFindPlugin(ID_CDUNLINKOBJECTS,CD_COMMAND_PLUGIN)) return true;
	...
	if(!reg) return CDRegisterCommandPlugin(ID_CDUNLINKOBJECTS,name,PLUGINFLAG_HIDE,"CDUnLinkObjects.TIF","CD Unlink Objects",CDDataAllocator(CDUnLinkObjectsR));
	else return CDRegisterCommandPlugin(ID_CDUNLINKOBJECTS,name,0,"CDUnLinkObjects.TIF","CD Unlink Objects",CDDataAllocator(CDUnLinkObjects));
}
