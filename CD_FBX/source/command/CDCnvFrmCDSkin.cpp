	...
	if(res)
	{
		...
		GroupBegin(0,BFH_SCALEFIT,1,0,"",0);
		{
			...
			GroupBegin(0,BFH_SCALEFIT,1,0,GeLoadString(DLG_CDJS_OPTIONS),0);
			{
				...
				AddChild(IDC_CDJS_OPTIONS_RB, 0, GeLoadString(IDS_CAWEIGHTS));
				AddChild(IDC_CDJS_OPTIONS_RB, 1, GeLoadString(IDS_CBWEIGHTS));
...
{
	if(CDFindPlugin(1019259,CD_COMMAND_PLUGIN) && CDFindPlugin(ID_CDCONVERTFROMCDSKIN,CD_COMMAND_PLUGIN)) return true;
